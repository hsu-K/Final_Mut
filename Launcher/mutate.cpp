...
// 產生Preventive Mutations，主要用於對抗VM偵測
BOOL GenPreventiveMutationsAll()
{
	// 初始化一個recording，並設定其origin為0，用於函數呼叫的描述
	Recording rec;
	rec.origin = 0;
	...
	// 設定當遇到NtOpenKey時，若Context為"vbox"或"virtual"，則返回STATUS_OBJECT_NAME_NOT_FOUND等失敗的結果
	rec.call = Call::cNtOpenKey;
	rec.type = CTX_SUB;
	wcscpy_s(rec.value.szCtx, MAX_CTX_LEN, L"vbox");
	mutType = MUT_FAIL;		// 返回結果「失敗」
	mutVal.nValue = 0xC0000034; // STATUS_OBJECT_NAME_NOT_FOUND	返回錯誤碼
...
// 生成Responsive Mutations，如果能生成的話，返回TRUE
// Responsive和Preventive的區別在於Responsive是針對當前執行時遇到的Call進行修改
